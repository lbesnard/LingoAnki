...
def backfill_lesson_last_reviewed(diary_json_path: str) -> None:
    ...
    if modified:
        ...
        logging.info(
            f"Backfilled last_reviewed for {len(diary_data.get('diaries', []))} days in {diary_path}"
        )
    else:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Backfill last_reviewed field in diary.json files"
    )
    parser.add_argument(
        "--config-root",
        default=os.getenv("CONFIG_ROOT", "~/.config/lingoDiary"),
